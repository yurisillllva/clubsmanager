...
function ClubList() {
  // Estado corrigido para armazenar resposta completa da API
  ...
  const loadClubs = async () => {
    try {
      ...
      // Ajuste crítico: a resposta já contém data e meta
      setApiResponse({
  ...
  // useEffect corrigido com dependências adequadas
  useEffect(() => {
    loadClubs();
  }, [page, search]); // Busca dados quando página ou busca mudam

  const handleDelete = async (id) => {
    if (window.confirm('Tem certeza que deseja excluir este clube?')) {
      await deleteClub(id);
      await loadClubs(); // Força recarregamento após exclusão
    }
  };
          ...
          {apiResponse.data?.length > 0 ? (
            apiResponse.data.map(club => (
              <tr key={club.id}>
                ...
                <td>
                  {/* Formatação de telefone mais segura */}
                  {club.telefone 
